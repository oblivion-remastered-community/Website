...
export async function POST(req: NextRequest) {
    ...
    const teamIds = teamsRaw.filter(t => t !== 'unselected').map(t => parseInt(t.toString()));
    if (!teamIds.every(t => !isNaN(t))) return NextResponse.json({statusText: 'Invalid `teams[]` form data.'}, { status: 422 })
